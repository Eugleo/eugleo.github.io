...
Žijete v jednorozměrném městě, které leží na úsečce. V městě je několik podniků, kde se čepuje pivo; každý podnik čepuje jen nednu značku. Tomu odpovídá první řádek ve vstupním souboru, například ze vstupu:

```
ABCDBC
```

lze vyčíst, že ve městě máte šest hospod, z nichž první točí značku `A`, druhá značku `B`, třetí značku `C` a tak dále.
