export const metadata = {
  ...
  description: 'Seznamy, Maybe, Either',
...
5. Napište instanci `Box` pro typ `(r ->)`, někdy také psaný jako `(->) r`. Jinými slovy, funkce (o jednom argumentu, jiná ani neexistuje) se zafixovaným typem tohoto argumentu, ale ne návratovým typem.
...
```
-- Jestliže nějaký typ b je PPL, pak typ b je i Box
...
3. Implementujte funkci `altLift` pomocí funkce `logistics`. Měla by "dělat" to samé jako `lift` (její typ bude samozřejmě trochu jiný, když vyžadujeme `logistics`).
