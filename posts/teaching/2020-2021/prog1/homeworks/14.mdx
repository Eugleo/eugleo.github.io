...
Neúplné určení nebudu uznávat. Například tedy, pokud bych se ptal na určení detailů `f` definovaného jako `f x = x + 1` tak chci odpověď: `f` je hodnota, `f :: Int -> Int` (neboli `f` má typ `Int -> Int`). Dejte **pozor** na detaily: například výraz `f 10` je také hodnota, jako `f`, ale má už jiný typ — `Int`.
