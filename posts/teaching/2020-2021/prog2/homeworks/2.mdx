...
Vaším úkolem je nyní napsat funkci `table(expr)`, která jako argument dostane nějaký takový logický výraz (tj. funkci, která bere dva argumenty) a vypíše jeho _pravdivostní tabulku_, tedy jeho hodnotu pro všechny kombinace `x` a `y`. Taková tabulka bude mít vždy čtyři řádky, které budou odpovídat jednotlivým kombinacím `True` a `False` dosazeným za `x` a `y` a tři sloupce; první bude vyjadřovat hodnotu `x`, druhý hodnotu `y` a třetí hodnotu `expr(x, y)` — tj. hodnotu logických výrazu, do kterého je dosazeno `x` a `y`.
